...
const BATCH_LIMIT = 200; // First 200 only
...
async function main() {
    ...
    try {
        ...
        // Get IDs of those who wrote the exam (have responses)
        const responders = await prisma.response.findMany({
            where: { userId: { in: allApplicants.map(u => u.id) } },
            select: { userId: true },
            distinct: ['userId']
        });
        ...
        // Take first BATCH_LIMIT
        const batch = unique.slice(0, BATCH_LIMIT);

        console.log(`   Did NOT write exam: ${notWrote.length}`);
        console.log(`   Unique emails: ${unique.length}`);
        console.log(`   This batch (first ${BATCH_LIMIT}): ${batch.length}`);
