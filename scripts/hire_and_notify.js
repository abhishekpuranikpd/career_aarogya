...
async function main() {
    ...
    const TEST_EMAILS = ["phd.drshubham@gmail.com", "abhishekpuranikpd@gmail.com"];

    try {
        // Fetch default job title safely
        ...
        if (!isExecute) {
            ...
            users.slice(0, 5).forEach(u => {
                console.log(` - ${u.name} (${u.email}) -> Will update to HIRED | Job: ${defaultJobTitle}`);
            });
            return;
        }
