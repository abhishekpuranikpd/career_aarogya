          ...
          <div className="container mx-auto">
            <div className="flex flex-col md:flex-row justify-between items-end gap-6">
              ...
              <div className="flex gap-4">
                 <Link 
                   href={`/register?jobId=${job.id}&title=${encodeURIComponent(job.title)}`}
                   className="px-8 py-4 bg-white text-gray-900 font-bold rounded-full hover:bg-blue-50 transition shadow-lg text-lg text-center min-w-[160px]"
                 >
                   Apply Now
                 </Link>
      ...
      <div className="container mx-auto px-4 py-12 grid md:grid-cols-3 gap-12">
        ...
        <div className="space-y-6">
           ...
           <div className="bg-gray-50 p-6 rounded-xl border border-gray-100 sticky top-24">
             ...
             <Link 
               href={`/register?jobId=${job.id}&title=${encodeURIComponent(job.title)}`}
               className="block w-full py-3 bg-primary text-white text-center font-bold rounded-lg mt-6 hover:bg-blue-700 transition"
             >
               Apply Now
             </Link>
           </div>
